...
2. **Trained machine-learning models** — provided as persisted artifacts in [`models/`](models/) (XGBoost booster
   JSON; logistic-regression and multilayer-perceptron `joblib` files; plus the imputation medians and the exact
   encoded-column order in `models/preprocessing.json`). They are regenerated by `scripts/export_models.py`, which
   verifies each artifact against `results/model_results.json` (|Δ AUC| ≤ 1e-4) and against the archived test-set
   predictions (`results/test_proba_*.npy`, identical probabilities). See `models/README.md` for load/apply snippets.
